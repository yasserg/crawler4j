...
public final class UrlResolver {
     ...
     * <a href="http://www.faqs.org/rfcs/rfc1808.html">RFC1808</a> Section 4 for more details.
     *
     * @param baseUrl
    ...
    private static Url resolveUrl(final Url baseUrl, final String relativeUrl) {
        ...
        while ((pathSegmentIndex = path.indexOf("/./")) >= 0) {
            path = path.substring(0, pathSegmentIndex + 1).concat(path.substring(pathSegmentIndex
                    + 3));
        }
        ...
        while ((pathSegmentIndex = path.indexOf("/../")) > 0) {
            ...
            if (!"..".equals(pathSegment.substring(slashIndex))) {
                path = path.substring(0, slashIndex + 1).concat(path.substring(pathSegmentIndex
                        + 4));
            }
        }
