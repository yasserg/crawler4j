...
public class Counters extends Configurable {
    ...
    public Counters(Environment env, CrawlConfig config) {
        ...
        /*
         * When crawling is set to be resumable, we have to keep the statistics in a transactional
         * database to make sure they are not lost if crawler is crashed or terminated unexpectedly.
         */
        if (config.isResumableCrawling()) {
            ...
            try (Cursor cursor = statisticsDB.openCursor(tnx, null);) {
        ...
        synchronized (mutex) {
            try {
                ...
                if (statisticsDB != null) {
                    ...
                    statisticsDB.put(txn, new DatabaseEntry(name.getBytes()), new DatabaseEntry(Util
                            .long2ByteArray(value)));
                    txn.commit();
                }
            } catch (Exception e) {
