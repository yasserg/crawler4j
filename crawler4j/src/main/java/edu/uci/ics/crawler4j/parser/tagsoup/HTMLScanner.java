...
public class HTMLScanner implements Scanner, Locator {
    ...
    private static final String[] debug_actionnames = {"", "A_ADUP", "A_ADUP_SAVE", "A_ADUP_STAGC", "A_ANAME",
        ...
        "A_MINUS", "A_MINUS2", "A_MINUS3", "A_PCDATA", "A_PI", "A_PITARGET", "A_PITARGET_PI", "A_SAVE",
    ...
    */
    ...
    public void scan(Reader r0, ScanHandler h) throws IOException, SAXException {
        ...
        while (theState != S_DONE) {
            ...
            curr = ch;
            next = r.read();
            unread(r, next); // read the next character and put it back
            ...
            switch (action) {
                ...
                case A_ENTITY:
                    mark();
                    char ch1 = (char)ch;
                    // System.out.println("Got " + ch1 + " in state " +
                    // ((theState == S_ENT) ? "S_ENT" : ((theState == S_NCR) ? "S_NCR" : "UNK")));
                    ...
                    if (ent != 0) {
                        ...
                        if (ent < 0x20) {
                            ...
                            } else if ((markPrev != 0 && markPrev > 0x20) && //Check for printable char preceding entity
                                (markNext != 0 && markNext > 0x20)) { //Check for printable char after entity
                                loadBuffWithStash(theOutputBuffer, savedOffset, savedSize, stash);
                                theOutputBuffer[savedSize] = (char) ent; // add entity to end of current word fragment
                                // Restore parsing state to before entity encountered
                                theSize = ++savedSize; // Accounting for one entity character added to theOutputBuffer
                            ...
                            } else if ((markPrev != 0 && markPrev > 0x20) && //Checks for printable char before entity
                                (markNext != 0 && markNext <= 0x20)) { // Check for whitespace after entity
                                loadBuffWithStash(theOutputBuffer, savedOffset, savedSize, stash);
                                theOutputBuffer[savedSize] = (char) ent; // add entity to end of current word fragment
                                // Restore parsing state to before entity encountered
                                theSize = ++savedSize; // Accounting for one entity character added to theOutputBuffer
                            ...
                            // handles the case where entity is not in a word but stands alone
                            } else if ((markPrev != 0 && markPrev <= 0x20) && //Check for whitespace before entity
                                (markNext != 0 && markNext <= 0x20)) { // Checks for whitespace after entity
                                loadBuffWithStash(theOutputBuffer, savedOffset, savedSize, stash);
                                theOutputBuffer[savedSize] = (char) ent; // add entity to end of current word fragment
                                // Restore parsing state to before entity encountered
                                theSize = ++savedSize; // Accounting for one entity character added to theOutputBuffer
                                ...
                                logger.error(
                                    "The HTMLScanner failed to integrate entity in A_ENTITY" +
                                    "action for entity with decimal value: "
                                    + ent);
                            }
